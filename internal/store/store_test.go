...
func TestOpenAndMigrate(t *testing.T) {
	...
	// Verify schema version is 22
	v := s.getUserVersion()
	if v != 22 {
		t.Errorf("expected schema version 22, got %d", v)
	}
...
func TestV21IntegrityMigrationNullsInvalidReferences(t *testing.T) {
	...
	if reopened.SchemaVersion() != 22 {
		t.Fatalf("schema version = %d, want 22", reopened.SchemaVersion())
	}
